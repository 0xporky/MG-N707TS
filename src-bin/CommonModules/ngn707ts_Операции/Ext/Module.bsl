...
//	Структура из полями статуса апарата.
//
...
Функция ОткрытьКассовуюСмену(ПараметрыПодключения) Экспорт
	...
	Заголовки = ПолучитьСоответствиеЗаголовков();
...
//		* Пароль - пароль под которым пользователь выполняет операции
//				   из кассовым аппаратом.
//	ТаблицаНоменклатуры - таблица значений, которая содержит колонки:
//		* Цена 						- Число 	- цена товара для которого необходимо пробить чек.
//		* Количество				- Число 	- колчество товара для которое необходимо перенести в чек.
...
//		* ЕстьСкидка				- ngn707ts_ТипыСкидок	- признак использования скидок / наценок.
...
//		* НомерТранзакции - Строка - идентификатор банковской транзакции (из ьанковского терминала).
...
Функция ПолучитьТаблицуНалогов(ПараметрыПодключения) Экспорт
...
Функция ПолучитьТаблицуВидовОплаты(ПараметрыПодключения) Экспорт
...
Функция ПолучитьСтруктуруОплаты() Экспорт
...
Функция АнализОтветаКассовогоАппарата(Ответ)
	...
	СтруктураОтвета = ПолучитьСтруктуруАнализаОтвета();
...
// Строит соответствие кодов ошибок, которые есть в прошывке кассового аппарата.
//
// Возвращаемое значение:
...
//		* ТекстОшибки_UA	- Cтрока - описание ошибки на украинском языке.
//
Функция ПолучитьТаблицуОшибок()
	...
	// Для  расшифровки результирующих данных необходимо реализровать алгоритм inflate
...
Функция ПроверитьКодОтветаКА(Ответ)
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ТекстИсключения = НСтр("ru='Возникла неопознанная ошибка в кассовом аппарате. Служебное сообщение: '; uk='Виникла неопізнанна помилка в касовому апараті. Службове повідомлення: '");
