...
//				   из кассовым аппаратом.
//
Функция СтруктураПараметровПодключения() Экспорт
...
// выполнении операций из кассовым аппаратом. Заголовки необходимо сохранить 
...
//		* Пароль 	 - Строка - пароль под которым пользователь выполняет операции
//				   	   из кассовым аппаратом.
//	URL 	- Строка - адрес по которому будет выполняться запрос.
//	Метод 	- Строка - метод которые будет выполняться на кассовом аппарате ( GET, POST ... ).
//
// Возвращаемое значение:
//  Структура - содержит свойства:
//		* realm  - Строка - служебный заголовок realm.
//		* nonce  - Строка - служебный заголовок nonce.
//		* opaque - Строка - служебный заголовок nonce.
...
// Возвращает заголовок для прохождения авторизации на кассовым аппарате.
//
// Параметры:
//	ПараметрыПодключения - Структура - содержит свойства:
//		* Сервер - Строка - имя или ip-адрес кассового апарата.
...
//		* Пароль - Строка - пароль под которым пользователь выполняет операции
//				   из кассовым аппаратом.
//  	* СлужебныеЗаголовки - Структура - структура которая содержит свойства:
...
//
// Возвращаемое значение:
//	Строка - содержит заголовок для авторизации на кассовом аппарате.
//
Функция ПолучитьЗаголовокДайджестАвторизации(ПараметрыПодключения, URL, Метод) Экспорт
	...
	nonceCount	= "00000001";
	cnonce		= НРег(ngn707ts_Служебный.ПолучитьХешМД5(Строка(ТекущаяДатаСеанса()))); // генерируем псевдо случайные значения для ключа
	...
	HA1 = ngn707ts_Служебный.ПолучитьХешМД5(А1);
	HA2 = ngn707ts_Служебный.ПолучитьХешМД5(А2);
	...
	response = НРег(ngn707ts_Служебный.ПолучитьХешМД5(response));
...
Процедура Заполнить_realm_nonce(ЗаголовокАутентификации, realm, nonce, opaque)
	...
	МассивПодзаголовков = СтрРазделить(ЗаголовокАутентификации, ",");
	
	Для каждого ПодзаголовокИзМассива Из МассивПодзаголовков Цикл
